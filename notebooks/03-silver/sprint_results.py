...
# COMMAND ----------
current_input("sprint_results").createOrReplaceTempView("source_rows")
clean = spark.sql(f"""
SELECT
...
spark.sql(f"""
CREATE TABLE IF NOT EXISTS {silver}.sprint_results USING DELTA
AS SELECT * FROM clean_rows WHERE 1 = 0
""")
spark.sql(f"""
MERGE INTO {silver}.sprint_results t
USING clean_rows s
...
""")
check_rows(spark.table(f"{silver}.sprint_results"), ["race_id","driver_id","car_number"], ["race_id","season","round","driver_id","constructor_id","car_number","finish_status"], clean.count())
