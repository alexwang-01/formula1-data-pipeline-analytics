...
# COMMAND ----------
current_input("driver_standings").createOrReplaceTempView("source_rows")
clean = spark.sql(f"""
SELECT
...
""")
check_rows(clean, ["season","driver_id"], ["season","driver_id","standing_status","championship_won"])
...
spark.sql(f"""
CREATE TABLE IF NOT EXISTS {silver}.driver_standings USING DELTA
AS SELECT * FROM clean_rows WHERE 1 = 0
""")
spark.sql(f"""
MERGE INTO {silver}.driver_standings t
USING clean_rows s
ON t.season = s.season AND t.driver_id = s.driver_id
WHEN MATCHED THEN UPDATE SET *
WHEN NOT MATCHED THEN INSERT *
WHEN NOT MATCHED BY SOURCE THEN DELETE
""")
check_rows(spark.table(f"{silver}.driver_standings"), ["season","driver_id"], ["season","driver_id","standing_status","championship_won"], clean.count())
