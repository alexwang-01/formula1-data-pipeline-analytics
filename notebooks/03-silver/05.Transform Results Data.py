...
# MAGIC 1. Standardise column names using snake_case (`constructorId` to `constructor_id`, `driverId` to `driver_id`, `raceName` to `race_name`, `positionText` to `finish_position_text`)
# MAGIC 1. Rename columns to make them more meaningful (`date` to `race_date`, `grid` to `grid_position`, `laps` to `completed_laps`, `number` to `car_number`, `position` to `finish_position`)
