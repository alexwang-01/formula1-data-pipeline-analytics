...
source_path = f"{landing}/csv/race_results"
raw = (
    ...
    .option("cloudFiles.schemaLocation", f"{state_root}/schema/race_results")
...
query = (
    ...
    .outputMode("append")
    .option("checkpointLocation", f"{state_root}/checkpoints/race_results")
    .option("mergeSchema", "true")
    .trigger(availableNow=True)
    .toTable(f"{bronze}.race_results")
)
...
# COMMAND ----------
selected = current_input("race_results")
print(f"race_results: {selected.count()} rows available for {release_tag}")
