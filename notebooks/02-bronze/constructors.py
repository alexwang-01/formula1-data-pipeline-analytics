...
source_path = f"{landing}/csv/constructors"
raw = (
    ...
    .option("cloudFiles.schemaLocation", f"{state_root}/schema/constructors")
...
query = (
    ...
    .outputMode("append")
    .option("checkpointLocation", f"{state_root}/checkpoints/constructors")
    .option("mergeSchema", "true")
    .trigger(availableNow=True)
    .toTable(f"{bronze}.constructors")
)
...
# COMMAND ----------
selected = current_input("constructors")
print(f"constructors: {selected.count()} rows available for {release_tag}")
