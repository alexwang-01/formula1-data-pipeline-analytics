...
source_path = f"{landing}/csv/driver_standings"
raw = (
    ...
    .option("cloudFiles.schemaLocation", f"{state_root}/schema/driver_standings")
...
query = (
    ...
    .outputMode("append")
    .option("checkpointLocation", f"{state_root}/checkpoints/driver_standings")
    .option("mergeSchema", "true")
    .trigger(availableNow=True)
    .toTable(f"{bronze}.driver_standings")
)
...
# COMMAND ----------
selected = current_input("driver_standings")
print(f"driver_standings: {selected.count()} rows available for {release_tag}")
