...
source_path = f"{landing}/csv/races"
raw = (
    ...
    .option("cloudFiles.schemaLocation", f"{state_root}/schema/races")
...
query = (
    ...
    .outputMode("append")
    .option("checkpointLocation", f"{state_root}/checkpoints/races")
    .option("mergeSchema", "true")
    .trigger(availableNow=True)
    .toTable(f"{bronze}.races")
)
...
# COMMAND ----------
selected = current_input("races")
print(f"races: {selected.count()} rows available for {release_tag}")
