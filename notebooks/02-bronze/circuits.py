...
source_path = f"{landing}/csv/circuits"
raw = (
    ...
    .option("cloudFiles.schemaLocation", f"{state_root}/schema/circuits")
...
query = (
    ...
    .outputMode("append")
    .option("checkpointLocation", f"{state_root}/checkpoints/circuits")
    .option("mergeSchema", "true")
    .trigger(availableNow=True)
    .toTable(f"{bronze}.circuits")
)
...
# COMMAND ----------
selected = current_input("circuits")
print(f"circuits: {selected.count()} rows available for {release_tag}")
