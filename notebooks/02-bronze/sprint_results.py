...
source_path = f"{landing}/csv/sprint_results"
raw = (
    ...
    .option("cloudFiles.schemaLocation", f"{state_root}/schema/sprint_results")
...
query = (
    ...
    .outputMode("append")
    .option("checkpointLocation", f"{state_root}/checkpoints/sprint_results")
    .option("mergeSchema", "true")
    .trigger(availableNow=True)
    .toTable(f"{bronze}.sprint_results")
)
...
# COMMAND ----------
selected = current_input("sprint_results")
print(f"sprint_results: {selected.count()} rows available for {release_tag}")
