...
source_path = f"{landing}/csv/drivers"
raw = (
    ...
    .option("cloudFiles.schemaLocation", f"{state_root}/schema/drivers")
...
query = (
    ...
    .outputMode("append")
    .option("checkpointLocation", f"{state_root}/checkpoints/drivers")
    .option("mergeSchema", "true")
    .trigger(availableNow=True)
    .toTable(f"{bronze}.drivers")
)
...
# COMMAND ----------
selected = current_input("drivers")
print(f"drivers: {selected.count()} rows available for {release_tag}")
